...
$lang = array_merge($lang, array (
  ...
  'Stats_text' => 'Összesen {total_files} fájl {total_categories} kategóriában<br />Eddig {total_downloads} letöltés történt<br /><br />A legújabb fájl: <a href={u_newest_file}>{newest_file}</a><br />A legrégebbi fájl: <a href={u_oldest_file}>{oldest_file}</a><br /><br />A fájlok átlagos értékelése: {average}/10<br />A legnépszerûbb fájl a <a href={u_popular}>{popular}</a> , {most}/10-re értékelve<br />A legkevésbé népszerû fájl a <a href={u_lpopular}>{lpopular}</a>, {least}/10-re értékelve<br /><br />Egy fájlra jutó letöltési arány: {avg_dls}<br />A legnépszerûbb fájlletöltés a <a href={u_most_dl}>{most_dl}</a> összesen {most_no} letöltéssel<br />A legkevésbé népszerû fájlletöltés a <a href={u_least_dl}>{least_dl}</a> összesen {least_no} letöltéssel<br />',
))
?>